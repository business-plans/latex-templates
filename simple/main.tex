  ...
  {% for name, content in CommercialEstate.items() %}
    ...
    \begin{enumerate}
      ...
      \item {{ equipmentName }}
        ...
        {% if key!= 'getPrices' %}\item {{ key }}: {{ value }}{% endif %}{% endfor %}
        \end{itemize}
      {% endfor %}
    \end{enumerate}
    {% endif %}
    {% if 'Worker' in content %}
      ...
      {{ role }}
        {% for key, value in params.items() %}
        {% if key!= 'getPrices' %}{{ key }}: {{ value }}{% endif %}{% endfor %}
      {% endfor %}
    {% endif %}
    ...
    {% set l=[] %}
    {% set residue=0 %}
    {% for name, value in content.getPrices.distribution.equipments.monthly.items() %}{%set tmp=(100*value/content.getPrices.perCategory.monthly['Оборудование'])|round%}
    {% if tmp>3 and l.append( tmp|string + "/" + name|string ) %}{% set residue+=tmp %}{% endif %}{% endfor %}
    \begin{tikzpicture}
    \pie [rotate = 180]
    {
        {{ l | join(", ") }},{{ residue }}/Остальное
    }
    \end{tikzpicture}
    {% endif %}
  {% endfor %}
...
{% if MaterialsConversion is defined and MaterialsConversion|length %}
  {% for name, content in MaterialsConversion.items() %}
      ...
      \item {{ mat.substanceTags | join(" ") }} в количестве {{ mat.amount }}{% endfor %}
      \end{itemize}
      {% endif %}
  {% endfor %}
{% endif %}
