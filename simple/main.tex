...
{% if CommercialEstate is defined and CommercialEstate|length %}
  {% for name, content in CommercialEstate.items() %}
    ...
    {% for name, value in content.getPrices.distribution.equipments.monthly.items() %}
      {% set tmp=100*value/content.getPrices.perCategory.monthly['Оборудование'] %}{% if tmp>5 %}{% if l.append( tmp|round|int|string + "/" + name|string )%}{% endif %}
      {% else %}{% if vv.append(tmp) %}{% endif %}{% endif %}
    {% endfor %}
    \begin{tikzpicture}
    \pie [rotate = 180]
    {
