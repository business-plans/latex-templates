...
{% if CommercialEstate is defined and CommercialEstate|length %}
  {% for name, content in CommercialEstate.items() %}
    ...
    {% set l=[] %}
    {% for name, value in content.getPrices.distribution.equipments.monthly.items() %}{% if l.append( (100*value/content.getPrices.perCategory.monthly['Оборудование'])|int|string + "/" + name|string ) %}{% endif %}{% endfor %}
    \begin{tikzpicture}
    \pie [rotate = 180]
    {
        {{ l | join(", ") }}
