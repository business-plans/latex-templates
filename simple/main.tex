...
{% if CommercialEstate is defined and CommercialEstate|length %}
  {% for name, content in CommercialEstate.items() %}
    ...
    {% for name, value in content.getPrices.distribution.equipments.monthly.items() %}
      {% set tmp=(100*value/content.getPrices.perCategory.monthly['Оборудование'])|round %}
      {% if tmp>3 and l.append( tmp|string + "/" + name|string ) %}
        {% if residue=residue+tmp %} {% endif %}
      {% endif %}
    {% endfor %}
    \begin{tikzpicture}
    \pie [rotate = 180]
    {
        {{ l | join(", ") }}, {{ 100-residue }}/Остальное
    }
    \end{tikzpicture}
    {% endif %}
  {% endfor %}
