  ...
  {% for name, content in CommercialEstate.items() %}
    ...
    \begin{enumerate}
      ...
      \item {{ equipmentName }}
        ...
        {% if key!= 'getPrices' %}\item {{ key }}: {{ value }}{% endif %}
        {% endfor %}
        \end{itemize}
      {% endfor %}
    \end{enumerate}
    ...
    {% if 'Worker' in content %}
      ...
      {{ role }}
        {% for key, value in params.items() %}
        {% if key!= 'getPrices' %}{{ key }}: {{ value }}{% endif %}
        {% endfor %}
      {% endfor %}
    {% endif %}
    ...
    {% set l=[] %}
    {% for name, value in content.getPrices.distribution.equipments.monthly.items() %}{% if l.append( value|string + "/" + name|string ) %}{% endif %}{% endfor %}
    \begin{tikzpicture}
    \pie [rotate = 180]
    {
        {{ l | join(", ") }}
